            ...
            match nvml.device_by_index(0) {
                Ok(device) => {
                    match device.memory_info() {
                        ...
                        Err(_) => println!("GPU VRAM : [Failed to read memory info]"),
                    }
                }
                Err(_) => println!("GPU VRAM : [No CUDA device found]"),
            }
        }
        Err(_) => println!("GPU VRAM : [NVML not available]"),
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    // Initialize logging - defaults to INFO level, can be controlled by RUST_LOG env var
    ...
    if let Some(process) = sys.process(sysinfo::Pid::from_u32(std::process::id())) {
        ...
        println!("System RAM: {:.2} MB", memory_mb);
    } else {
        println!("System RAM: Unable to retrieve process information");
    }
