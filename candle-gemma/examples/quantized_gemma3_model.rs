...
impl RmsNorm {
    ...
    fn from_qtensor(qtensor: QTensor, eps: f64) -> Result<Self> {
        let scale = qtensor.dequantize(&Device::Cpu)?;
        Ok(Self::new(scale, eps))
    }

    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        candle_nn::ops::rms_norm(x, &self.scale, self.eps as f32)
    }
}
...
struct LayerWeights {
    ...
    kv_cache_dtype: DType, // Added: control cache dtype
...
impl LayerWeights {
    fn mask(
        ...
        let mask = if index_pos > 0 {
            let mask0 = Tensor::zeros((seq_len, index_pos), dtype, device)?;
    ...
    fn forward_attn(
        ...
        let k = k.to_dtype(self.kv_cache_dtype)?;
        let v = v.to_dtype(self.kv_cache_dtype)?;
        ...
        // Cast back to standard dtype (f32) if cache is lower precision
        let k = k.to_dtype(q.dtype())?;
        let v = v.to_dtype(q.dtype())?;
...
impl ModelWeights {
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        reader: &mut R,
        device: &Device,
        kv_cache_dtype: DType, // Added: kv cache dtype argument
        max_seq_len: usize,    // Added: explicit max seq len instead of constant
    ) -> Result<Self> {
        ...
        for layer_idx in 0..block_count {
            ...
            let rotary_embedding = RotaryEmbedding::new(key_length, layer_rope_frequency, max_seq_len, device)?;
            ...
            layers.push(LayerWeights {
                ...
                kv_cache: None,
                kv_cache_dtype, // Use provided dtype
                span_attn,
                span_mlp,
            })
        }
