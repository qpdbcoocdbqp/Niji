...
struct Args {
    ...
    #[arg(long, default_value = "gemma3-1b-it")]
    which: Which,
}
...
impl Args {
    fn tokenizer(&self) -> anyhow::Result<Tokenizer> {
        let tokenizer_path = match &self.tokenizer {
            Some(config) => expand_tilde(config),
            None => {
                let api = hf_hub::api::sync::Api::new()?;
                let repo = match self.which {
                    Which::Gemma3_4bIt => "google/gemma-3-4b-it",
                    Which::Gemma3_1bIt => "google/gemma-3-1b-it",
                };
    ...
    fn model(&self) -> anyhow::Result<std::path::PathBuf> {
        let model_path = match &self.model {
            ...
            None => {
                ...
                if let Ok(home) = std::env::var("MODEL_HOME") {
                    let path = std::path::PathBuf::from(home)
                        .join("huggingface/hub")
                        .join(format!("models--{}", repo.replace("/", "--")))
                        .join("snapshots/d1be121d36172a4b0b964657e2ee859d61138593")
                        .join(filename);
                    if path.exists() {
                        return Ok(path);
                    }
