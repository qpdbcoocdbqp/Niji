...
impl Args {
    fn tokenizer(&self) -> anyhow::Result<Tokenizer> {
        let tokenizer_path = &self.tokenizer;
    ...
    fn model(&self) -> anyhow::Result<std::path::PathBuf> {
        let model_path = self.model.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Model path is required. Use --model <path>"))?;
...
fn main() -> anyhow::Result<()> {
    ...
    std::env::set_var("CANDLE_DEQUANTIZE_ALL", "0");
    std::env::set_var("CANDLE_DEQUANTIZE_ALL_F16", "0");
    ...
    let mut model = {
        ...
        println!("model built in {:.2}s (device: {:?})", build_start.elapsed().as_secs_f32(), device);
        weights
    };
